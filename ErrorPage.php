...
// this array is for debugging purposes only
// array of possible error codes
$error_codes = array(400, 401, 402, 403, 404, 405, 406, 407, 408, 409,
  410, 411, 412, 413, 414, 415, 416, 417, 418, 421, 422, 423, 424,
  500, 501, 502, 503, 504, 505);

  //randomly select one of the error codes for each page load
  $default_code = $error_codes[array_rand($error_codes)];
...
switch ($code) {
  ...
  case 401:
    $default_code = 401;
    $default_message = "The request has not been applied because it lacks valid authentication credentials for the
target resource.";
    break;
  case 402:
    $default_code = 402;
  ...
  case 404:
    $default_code = 404;
    $default_message = "The webpage might be temporarily down or it may have moved permanently to a new web
address.";
    break;
  case 405:
    $default_code = 405;
  ...
  case 406:
    $default_code = 406;
    $default_message = "The requested resource is capable of generating only content not acceptable according to the
Accept headers sent in the request.";
  ...
  case 409:
    $default_code = 409;
    $default_message = "Indicates that the request could not be processed because of conflict in the request, such as
an edit conflict.";
    break;
  case 410:
    $default_code = 410;
    $default_message = "Indicates that the resource requested is no longer available and will not be available again.
This should be used when a resource has been intentionally removed and the resource should be purged.
Upon receiving a 410 status code, the client should not request the resource in the future. Clients
such as search engines should remove the resource from their indices.";
    break;
  case 411:
    $default_code = 411;
    $default_message = "The request did not specify the length of its content, which is required by the requested
resource.";
    break;
  case 412:
    $default_code = 412;
  ...
  case 415:
    $default_code = 415;
    $default_message = "The media format of the requested data is not supported by the server, so the server is
rejecting the request.";
    break;
  case 416:
    $default_code = 416;
    $default_message = "The range specified by the Range header field in the request can't be fulfilled; it's possible
that the range is outside the size of the target URI's data.";
  ...
  case 421:
    $default_code = 421;
    $default_message = "The request was directed at a server that is not able to produce a response (for example
...
request.";
  ...
  case 501:
    $default_code = 501;
    $default_message = "The server either does not recognize the request method, or it lacks the ability to fulfil the
request.";
    break;
  case 502:
    $default_code = 502;
    $default_message = "The server was acting as a gateway or proxy and received an invalid response from the
upstream server.";
    break;
  case 503:
    $default_code = 503;
    $default_message = "The server is temporarily unable to service your request due to maintenance downtime or
capacity problems. Please try again later.";
    break;
  case 504:
    $default_code = 504;
    $default_message = "The server was acting as a gateway or proxy and did not receive a timely response from the
upstream server.";
    break;
  case 505:
    $default_code = 505;
