        ...
        if ($_FILES["CrePic"]["size"] > 5000000) {
            ...
            if ($Password != $ConfirmPassword) {
                ...
                if (move_uploaded_file($_FILES["CrePic"]["tmp_name"], $target_file)) {
                    ...
                    $sql = "INSERT INTO tbl_admin (UID, name, admin_uname, admin_pword, admin_email, department, imagePath, date_created, last_login, role) VALUES ('$ID', '$name', '$username', '$Password', '$Email', '$department', '$target_file', '$date', '$date', '$Role')";
                    $result = mysqli_query($conn, $sql);
                    if ($result) {
                        if ($Role == "administrator") {
